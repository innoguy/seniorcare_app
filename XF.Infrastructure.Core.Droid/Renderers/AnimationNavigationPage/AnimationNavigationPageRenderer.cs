...
[assembly: ExportRenderer(typeof(NavigationPage), typeof(AnimationNavigationPageRenderer))]
namespace XF.Infrastructure.Core.Droid.Renderers
{
    public class AnimationNavigationPageRenderer : NavigationPageRenderer
    {
        private readonly AppCompatNavRendererHelper _hepler;

        public AnimationNavigationPageRenderer(Context context)
        : base(context)
        {
        ...
        {
            var index = ((INavigationPageController)Element).Pages.Count() - 1;
            // var pageToShow = ((INavigationPageController)Element).Pages.Skip(1).FirstOrDefault();
            Page pageToShow = null;
            if (index - 1 > 0)
            {
                pageToShow = ((INavigationPageController)Element).Pages.ElementAt(index - 1);
            }
            else
            {
                pageToShow = ((INavigationPageController)Element).Pages.Skip(1).FirstOrDefault();
            }
